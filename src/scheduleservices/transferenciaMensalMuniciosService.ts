...
export default class TransferenciaMensalMuniciosService {
    public async startProcess(year: string, month: string): Promise<void> {
        ...
        const transferenciasConstitucionaisMunicipiosPage = new TransferenciasConstitucionaisMunicipiosPage(page);
        await transferenciasConstitucionaisMunicipiosPage.openPage();

        console.log(`Obtendo tamanho atual do arquivo: ${fileName}`);
        const currentFileSize = await transferenciasConstitucionaisMunicipiosPage.getFileSizeByYearMonth(fileName);
        console.log(`Tamanho obtido: ${currentFileSize}`);


        console.log(`Comparando o tamanho do arquivo atual com o anterior...`);
        if(!await directusTransferenciaService.theFileSizeHasChanged(fileName, currentFileSize))
        ...
        const downloadLink = await transferenciasConstitucionaisMunicipiosPage.GetLinkTransferenciasMensalMunicipioFileByYearAndMonth(yearMonth);
        await browser.close();
        ...
        const arrayFromFileData = await csvService.materializeFromBuffer(buffer, 'iso-8859-1');

        arrayFromFileData.shift()
        const objectList = mapService.MapTranferenciaMensalMunicipioCsvFileToInteface(arrayFromFileData);
        console.log(`Dados preparados.`);

        console.log(`Deletando os dados anteriores...`);
        await directusTransferenciaMensalMunicipiosService.DeleteAllItemsByYearMonth(year, month);
        console.log(`Dados anteriores deletados com sucesso.`);

        console.log(`Iniciando o upload dos dados...`);
        await directusTransferenciaMensalMunicipiosService.UploadItemsTransferenciaMensalMunicipio(objectList);
        ...
        await directusTransferenciaService.createNewItem({nome_do_arquivo: fileName, tamanho: currentFileSize});
